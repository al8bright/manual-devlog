...
4. Name the file exactly `YYYYMMDD_[N]_제목.md`:
...
5. Write a self-contained Korean Markdown document with this required header format:
   ...
   - `# 검증한 내용`
   - `# 다음 단계`
6. Use Markdown structure actively in the body: use `##` for meaningful subtopics, `###` only for a further level of detail, `-` or numbered lists for repeated items, `**강조**` for key decisions, and short code fences only when they clarify a decision. Do not make every paragraph a heading or use headings merely for visual size.
7. After writing, report the file path, selected sequence number, and a short factual summary. State clearly that the file is a local draft and was not published.
